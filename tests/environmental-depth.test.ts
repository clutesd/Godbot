...
describe('environmental depth presentation', () => {
  ...
  it('deepens atmosphere and valley mist under severe weather without exploding density', () => {
    const clear = resolveEnvironmentalDepth({ daylight: 1, twilight: 0, weatherSoftening: 0, baseFogDensity: 0.0072, cameraHeight: 25 });
    const storm = resolveEnvironmentalDepth({ daylight: 0.45, twilight: 0.2, weatherSoftening: 1, baseFogDensity: 0.04, cameraHeight: 25 });
  ...
  it('tightens shadow coverage materially below the old fixed 75-unit half span', () => {
    const close = resolveEnvironmentalDepth({ daylight: 1, twilight: 0, weatherSoftening: 0, baseFogDensity: 0.0072, cameraHeight: 18 });
    const wide = resolveEnvironmentalDepth({ daylight: 1, twilight: 0, weatherSoftening: 0, baseFogDensity: 0.0072, cameraHeight: 90 });
  ...
  it('uses conservative contact-safe bias values', () => {
    const state = resolveEnvironmentalDepth({ daylight: 0.6, twilight: 0.4, weatherSoftening: 0.3, baseFogDensity: 0.01, cameraHeight: 35 });

...
export function societyFixture() {
  ...
  for (const cell of state.world.cells) Object.assign(cell, { elevation: ground, water: false, biome: 'grassland', landform: 'lowland', slope: 0, relief: 0,
    river: false, lake: false, coast: false, movementCost: 1, fertility: 0.75, moisture: 0.6, wood: 0.8, minerals: 0.5, naturalResources: undefined });
...
export function connect(state: SimulationState, a: Settlement, b: Settlement) {
  const segmentId = `fixture-road:${a.id}:${b.id}`;
  state.transportation.segments[segmentId] = { id: segmentId, from: a.id, to: b.id, mode: 'road', kind: 'surface', status: 'complete',
  ...
  state.tradeRoutes.push({ id: `${a.id}-${b.id}`, a: a.id, b: b.id, active: true, volume: 0.8, ageMonths: 100, mode: 'land', caravanProgress: 0,
    caravanDirection: 1, knowledgeFlow: 0, cumulativeKnowledge: 2, transport: { projectIds: [segmentId], nextDispatchMonth: 0,
      path: { mode: 'road', points: [{ ...a.position, y: 1 }, { ...b.position, y: 1 }], segmentIds: [segmentId], length: 36 } } });
}

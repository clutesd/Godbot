...
Use Node.js 22.12 or newer. The checked-in lockfile is for npm, so use `npm ci` for a fresh clone or Cloudflare build.
...
**Trees.** [`TreeLibrary.ts`](./src/render/vegetation/TreeLibrary.ts) grows seven families with three deterministic variants each. Deciduous branches grow breadth-first under a segment budget; needle trees have tapered leaders and layered whorls. Near/far tiers share the same seeded structure. Climate-aware cherry blossoms fade to summer green, autumn colors blend gradually, and cold deciduous crowns give way to bare winter branches. Tree size grows smoothly, ordinary stands regenerate, and significant ancient identities are never recycled.

**Flowers.** Small meadow colonies and woodland-edge flowers emerge, bud, bloom, set visible seed heads, and die back for winter. Cupped petals, paired leaves, pollen centers and seed heads use three bounded instanced pools. Frost, snow, flooding, terrain edges, building footprints and camera distance govern visibility. Falling petals originate at nearby flowering trees. See the [vegetation review and seasonal preview](./docs/VEGETATION_REPORT.md) for fixes, tests and visual QA limits.
...
**Purpose is a shape, not a colour.** Three further grammar fields carry social function into the silhouette. `frontage` decides how the building meets the street — open market stalls under an awning, a civic colonnade, a pedimented portico, a raised loading dock, an open working yard, a guarded screen, or an open ward arcade. `crown` gives the one large feature that survives to the skyline — spire, obelisk, lantern cupola, watch tower, observatory dome, flue cluster, cooling mass, water tank, or ridge monitor. `props` places a restrained set of deterministic exterior details: firewood and yard fences at houses, crates at markets, timber stacks at workshops, ore and slag at foundries, barrels at warehouses, herb beds at infirmaries, altars at sanctuaries, stakes and braziers at garrisons. All of it is emitted onto surfaces the building already draws.
...
**Material character.** [`SurfaceDetail`](./src/render/materials/SurfaceDetail.ts) gives each construction material its own small fragment program, injected into the shared surface material rather than painted as a texture: brick coursing and mortar rhythm, stone blocks with recessed joints, timber planking with darker joints and grain that follows the member's long axis, mottled and rain-streaked plaster, directional thatch courses, laid tile courses and caps, corrugated metal sheet, flush cladding panels with gasket seams. It is evaluated in the canonical building frame, so the pattern is deterministic, stable under instancing, and costs no extra draw calls, meshes or vertices. Mortar joints, tile courses and panel seams also perturb the normal, so the relief survives at close range.

**Age and use.** The grammar carries a seed-driven `wear` and `toneShift`; the composer bakes them into a four-byte per-vertex attribute and raises them locally for flues, forges and ground contact. Weathering therefore varies between neighbouring buildings and across one building's own fabric without changing its architecture. Civilisation creates the form; age and use only dull, soot and fade it.

**Performance.** Geometry is cached per `(culture, era, role, variation, stage)` rather than per instance, so a hundred houses share a handful of buffers; instances are clones over shared geometry and materials. Cached geometry and palette materials are flagged shared and are not disposed when an individual settlement group is rebuilt. Two LOD levels back each structure, and both restate frontage and crown so a market, a council, a garrison and a foundry stay different shapes at a distance.

**Developer gallery.** `tests/architecture-preview.html` renders the CI validation cases through the production `AssetBuilder`. `?view=street&case=trade` frames one structure at close range, `?full` pins full procedural detail, and `?report` shows shader errors, draw calls, triangles and asset cache stats.
...
- If Cloudflare installs with an older Node version, set the Pages environment variable `NODE_VERSION=22.12.0` and rebuild.

...
import { GodboxRenderer as BaseGodboxRenderer } from './GodboxRenderer';
...
/**
 * Transport-presentation specialization layered over the core renderer. The simulation remains
 * authoritative: this class only turns commissioned ports and completed bridge segments into
 * structures with believable physical grammar.
 */
export class GodboxRenderer extends (BaseGodboxRenderer as unknown as new (...args: any[]) => any) {
  ...
  private syncRoutes(force = false): void {
    ...
    for (const segment of Object.values(network.segments) as any[]) {
      ...
      self.routePlacementReports.set(segment.id, {
        ...
        waterSamples: bridge ? segment.points.length - 2 : 0,
        bridgeSegments: bridge ? 1 : 0,
        maxTerrainError: bridge ? 0 : segment.points.reduce((error: number, p: any) => Math.max(error, Math.abs(p.y - self.elevationAt(p.x, p.z) - 0.04)), 0),

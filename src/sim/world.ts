...
function sampleCell(
  ...
  return {
    elevation: read(height, index), slope: clamp01(gradient * 0.6), relief: highest - lowest, flow,
    rockiness: read(rock, index), lake, river,
  };
}
...
export function generateWorld(config: GodboxConfig, random = new SeededRandom(`${config.seed}:world`)): WorldState {
  ...
  const raw = synthesizeHeightfield({ seed: config.seed, size, cellSize, subdivision: TERRAIN_SUBDIVISION,
    noiseScale, seaLevel, mountainLevel, offsetX, offsetZ });
  ...
  const terrain: TerrainField = {
    resolution: raw.resolution, step: raw.step, originX: raw.originX, originZ: raw.originZ,
    height: raw.height, waterLevel: hydrology.waterLevel,
    floodDepth: new Float32Array(raw.height.length),
    flow: hydrology.flow, rock, lake: hydrology.lake, river: hydrology.river, fall: hydrology.fall,
  ...
  for (let z = 0; z < size; z += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      const movementCost = water ? 3.8 : 0.75 + elevation * 1.15 + sample.slope * 2.6 + (moisture > 0.7 ? 0.45 : 0) + (elevation > mountainLevel ? 1.5 : 0);
      const base = { x, z, worldX: (x - size / 2) * cellSize, worldZ: (z - size / 2) * cellSize,
        elevation, moisture, temperature, fertility, wood, minerals, habitability, movementCost, water,
        coast: false, river, lake, slope: sample.slope, relief: sample.relief, flow: sample.flow,
        rockiness: sample.rockiness, landform: landformFor(elevation, sample.slope, sample.relief, sample.flow, seaLevel, mountainLevel) };
  ...
  for (const cell of cells) {
    ...
    if (cell.coast) { cell.fertility = clamp01(cell.fertility + 0.08); cell.habitability = clamp01(cell.habitability + 0.1); }
  }
...
export function strategicSettlementCells(world: WorldState, count: number, random: SeededRandom): WorldCell[] {
  ...
  const landformBonus: Record<WorldCell['landform'], number> = {
    ocean: -1, shore: 0.05, lowland: 0.12, basin: 0.14, valley: 0.16, hill: 0.04,
    plateau: 0.06, ridge: -0.2, peak: -0.4, canyon: -0.3,
  };
  const candidates = world.cells.filter((cell) => !cell.water && cell.habitability > 0.38 && cell.slope < 0.28 && cell.elevation < 0.78)
    .map((cell) => ({ cell, score: cell.habitability + cell.fertility * 0.4 + (cell.coast ? 0.1 : 0)
      + (cell.flow > 0.4 ? 0.18 : 0) + landformBonus[cell.landform] + landmarkBonus(cell) + random.range(0, 0.16) }))
  ...
  for (let distance = initialDistance; distance >= world.cellSize * 3 && selected.length < count; distance *= 0.82) {
    for (const { cell } of candidates) {
      ...
      if (selected.every((other) => Math.hypot(cell.worldX - other.worldX, cell.worldZ - other.worldZ) >= distance)) selected.push(cell);
    }
  }

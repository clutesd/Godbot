...
function eventPriority(memory: CompressedEventMemory): number {
...
export class DeepHistoricalMemory {
  ...
  causalAssessment(sourceEventId: string, targetEventId: string): CausalAssessment {
    ...
    if (!source || !target || source.month >= target.month) return this.unknownAssessment(sourceEventId, targetEventId);
    ...
    return this.unknownAssessment(sourceEventId, targetEventId);
  }
  ...
  callbackFor(scene: ObservationCandidate, state: SimulationState): DeepHistoryRemark | undefined {
    ...
    const candidates = this.memory.events.filter((memory) => {
      if (activeEventIds.has(memory.eventId) || state.month - memory.month < 500 * 12 || memory.retrospectiveSignificance < 0.62) return false;
    ...
    if (chosen.retrospectiveSignificance - chosen.contemporarySignificance >= 0.18 && chosen.retrospectiveReasons.length > 0) {
      return {
        ...
        sourceMemoryIds: [chosen.id], provenance: 'historical-interpretation',
      };
    }
    return {
      text: `I remember this place ${years.toLocaleString()} years ago, when ${readable(chosen.type)} entered the record.`,
      sourceMemoryIds: [chosen.id], provenance: 'recorded-fact',
    };
  }
  ...
  private unknownAssessment(sourceEventId: string, targetEventId: string): CausalAssessment {
  ...
  private processEvent(event: HistoricalEvent, state: SimulationState): void {
    ...
    if (retainTarget) this.rememberEvent(event, this.reasonFor(event));

    for (const causeId of event.causes) {
      const cause = state.history.find((candidate) => candidate.id === causeId);
      if (cause) this.rememberEvent(cause, 'A later recorded event identifies it in its causal chain.');
      const hasCause = this.memory.events.some((item) => item.eventId === causeId);
      const hasTarget = retainTarget || this.memory.events.some((item) => item.eventId === event.id);
      if (hasCause && hasTarget) {
        this.rememberEvent(event, this.reasonFor(event));
  ...
  private rememberEvent(event: HistoricalEvent, whyRemembered: string): CompressedEventMemory {
  ...
  private addDirectCausalLink(fromEventId: string, toEventId: string): void {
    ...
    if (!this.memory.causalLinks.some((item) => item.id === id)) {
    ...
    const visit = (currentTargetId: string, depth: number, seen: Set<string>): void => {
      ...
      for (const link of this.memory.causalLinks.filter((item) => item.toEventId === currentTargetId)) {
        ...
        if (promoted > source.retrospectiveSignificance) {
          ...
          source.retrospectiveReasons = unique([
            ...source.retrospectiveReasons,
            `Later recorded consequences made this earlier ${readable(source.type)} more important in retrospect.`,
          ]).slice(-DEEP_HISTORY_LIMITS.reasonsPerRecord);
  ...
  private promoteKnowledgeLineage(memory: CompressedEventMemory): void {
    ...
    const origin = this.memory.events
    ...
    origin.retrospectiveReasons = unique([
      ...origin.retrospectiveReasons,
      `Later adoption of ${readable(memory.knowledgeId)} made this earlier contribution foundational in retrospect.`,
    ]).slice(-DEEP_HISTORY_LIMITS.reasonsPerRecord);
  ...
  private maybeCreateEpisode(memory: CompressedEventMemory, event: HistoricalEvent): void {
    ...
    const causeMonths = causeIds
      .map((id) => this.memory.events.find((item) => item.eventId === id)?.month)
      .filter((month): month is number => month !== undefined);
    this.upsertEpisode({
      id: `event-${event.id}`, title: memory.label,
      startMonth: Math.min(memory.month, ...causeMonths), endMonth: memory.month,
      entityIds: memory.entityIds, keyEventIds: [...causeIds, event.id],
      turningPointEventIds: TURNING_POINTS.has(event.type) ? [event.id] : [],
      consequences: event.outcome ? [event.outcome] : [], significance: Math.max(memory.contemporarySignificance, memory.retrospectiveSignificance),
    });
  }

  private upsertEpisode(input: DeepEpisodeInput): void {
    ...
    const record: DeepEpisodeMemory = {
      ...
      title: trimText(input.title, 120), startMonth: input.startMonth, endMonth: input.endMonth,
      ...
      provenance: {
        class: 'derived-statistic', sourceEventIds: unique(input.keyEventIds).slice(-DEEP_HISTORY_LIMITS.eventRefsPerRecord),
      ...
      whyRemembered: input.turningPointEventIds && input.turningPointEventIds.length > 0
        ...
        : 'It concentrates meaningful observations into a bounded episode.',
    };
  ...
  private updateEra(memory: CompressedEventMemory): void {
    ...
    if (!ERA_BOUNDARIES.has(memory.type) || createdNow) return;
    const minimumSpan = CIVILIZATION_SCALE_EVENTS.has(memory.type) ? 0 : 10 * 12;
    if (memory.month - current.startMonth < minimumSpan) return;
    current.endMonth = Math.max(current.startMonth, memory.month - 1);
  ...
  private newEra(startMonth: number, memory: CompressedEventMemory): DeepEraMemory {
    return {
      ...
      startMonth,
      structuralSignals: [{ type: memory.type, count: 1 }], keyMemoryIds: [memory.id],
  ...
  private compact(): void {
    this.memory.events.sort((a, b) => eventPriority(b) - eventPriority(a) || b.month - a.month || a.id.localeCompare(b.id));
  ...
  private compactEras(): void {
    this.memory.eras.sort((a, b) => a.startMonth - b.startMonth || a.id.localeCompare(b.id));
    if (this.memory.eras.length <= DEEP_HISTORY_LIMITS.eras) return;
    const first = this.memory.eras.slice(0, 8);
    const recent = this.memory.eras.slice(-32);
    const middle = this.memory.eras.slice(8, -32)
      .sort((a, b) => b.significance - a.significance || a.startMonth - b.startMonth)
      .slice(0, DEEP_HISTORY_LIMITS.eras - first.length - recent.length);
    const ids = new Set<string>();
    this.memory.eras = [...first, ...middle, ...recent]
      .filter((era) => !ids.has(era.id) && ids.add(era.id))
      .sort((a, b) => a.startMonth - b.startMonth || a.id.localeCompare(b.id));
  }

  private allMemoryIds(): Set<string> {
    return new Set<string>([
      ...this.memory.events.map((item) => item.id), ...this.memory.episodes.map((item) => item.id),
      ...this.memory.threads.map((item) => item.id), ...this.memory.eras.map((item) => item.id),
      ...this.memory.entities.map((item) => item.id), ...this.memory.civilization.map((item) => item.id),
    ]);
  }
...
/** Deep history is nested in the Watcher snapshot, which already persists through RunArchive. */

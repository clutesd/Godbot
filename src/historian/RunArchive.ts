...
export interface RunIdentity {
  ...
  createdAt: string;
  initialConditions: { population: number; settlements: number; cultures: number; worldSize: number };
}
...
export interface ArchivedPerson {
  id: string; name: string; cultureId: string; homeId: string; occupation: string; bornMonth: number;
  lastKnownMonth: number; lastKnownAgeYears: number; prestige: number; reasons: string[]; aliveAtLastRecord: boolean;
}
...
export interface RunArchiveRecord {
  ...
  majorEntities: { settlements: Settlement[]; cultures: Culture[]; polities: Polity[]; institutions: Institution[] };
  ...
  outcome: {
    population: number; peakPopulation: number; settlementsRemaining: number; industrialCenters: number;
    discoveries: number; knowledgeLost: number; rediscoveries: number; knowledgeExchanges: number;
    tradeRoutesEstablished: number; wars: number; classification: SimulationState['advanced']['outcome']['classification'];
    atomicThresholdMonth?: number; nuclearWeapons: boolean; nuclearWar: boolean; survivalYearsAfterAtomic: number | null;
    survivedThreeCenturiesAfterAtomic: boolean; interplanetary: boolean; postBiological: boolean; unknown: boolean; summary: string;
  };
  updatedAt: string;
}
...
export function configurationFingerprint(config: GodboxConfig): string {
  ...
  for (let index = 0; index < stable.length; index += 1) { hash ^= stable.charCodeAt(index); hash = Math.imul(hash, 16777619); }
...
export function createRunIdentity(config: GodboxConfig, state: SimulationState, observationNumber: number, createdAt = new Date().toISOString(), baseSeed = config.seed): RunIdentity {
  ...
  return {
    runId: `observation-${observationNumber}-${fingerprint}`, observationNumber, seed: state.seed,
    worldName: generatedWorldName(state.seed), configurationFingerprint: fingerprint,
    experimentFingerprint: experimentFingerprint(config), baseSeed, engineVersion: state.engineVersion, createdAt,
...
export class RunRecordBuilder {
  ...
  private capturePeople(state: SimulationState, representativeIds: ReadonlySet<string>): void {
    ...
    for (const person of state.people) {
      const reasons = [
        ...(representativeIds.has(person.id) ? ['representative'] : []), ...(historicallySignificantIds.has(person.id) ? ['historical-witness'] : []),
        ...(person.prestige >= 0.62 ? ['high-prestige'] : []), ...(person.ageMonths >= 72 * 12 ? ['long-life'] : []),
        ...(person.occupation === 'keeper' ? ['knowledge-keeper'] : []), ...(person.activity === 'migrate' ? ['migrant'] : []),
      ];
  ...
  private captureRecordedDeaths(state: SimulationState, representativeIds: ReadonlySet<string>): void {
    ...
    for (const event of state.history.filter((candidate) => candidate.type === 'death' && typeof candidate.context.name === 'string')) {
      const id = event.actors[0]; if (!id) continue;
      const age = Number(event.context.age ?? 0); const prestige = Number(event.context.prestige ?? 0); const occupation = String(event.context.occupation ?? '');
      const reasons = [
        ...(representativeIds.has(id) ? ['representative'] : []), ...(historicallySignificantIds.has(id) ? ['historically-significant'] : []),
        ...(age >= 72 ? ['long-life'] : []), ...(prestige >= 0.62 ? ['high-prestige'] : []), ...(occupation === 'keeper' ? ['knowledge-keeper'] : []),
      ];
      if (reasons.length === 0) continue;
...
export class HistorianArchiveStore {
  ...
  async initialize(): Promise<void> {
    ...
    } catch (error) { this.failureReason = error instanceof Error ? error.message : String(error); this.database = undefined; }
  }
  ...
  async nextObservationNumber(): Promise<number> {
    ...
    try {
      return await new Promise<number>((resolve, reject) => {
        ...
        const store = transaction.objectStore('metadata'); const request = store.get('observationNumber'); let next = 0;
        request.onsuccess = () => { next = Number(request.result ?? 0) + 1; store.put(next, 'observationNumber'); };
    ...
    } catch (error) { this.failureReason = error instanceof Error ? error.message : String(error); return ++this.memoryObservationNumber; }
  }

  async save(record: RunArchiveRecord): Promise<void> {
    const migrated = migrateArchiveRecord(record); this.memory.set(migrated.identity.runId, structuredClone(migrated));
    await this.initialize(); if (!this.database) return;
    try { await this.request<void>('runs', 'readwrite', (store) => store.put(migrated)); }
    catch (error) { this.failureReason = error instanceof Error ? error.message : String(error); }
  }

  async get(runId: string): Promise<RunArchiveRecord | undefined> {
    await this.initialize(); if (!this.database) return structuredClone(this.memory.get(runId));
    try { const raw = await this.request<unknown>('runs', 'readonly', (store) => store.get(runId)); return raw ? migrateArchiveRecord(raw) : undefined; }
    catch (error) { this.failureReason = error instanceof Error ? error.message : String(error); return structuredClone(this.memory.get(runId)); }
  }

  async list(): Promise<RunArchiveRecord[]> {
    await this.initialize(); if (!this.database) return [...this.memory.values()].map((record) => structuredClone(record));
    try { const raw = await this.request<unknown[]>('runs', 'readonly', (store) => store.getAll()); return raw.map(migrateArchiveRecord).sort((a, b) => a.identity.observationNumber - b.identity.observationNumber); }
    catch (error) { this.failureReason = error instanceof Error ? error.message : String(error); return [...this.memory.values()].map((record) => structuredClone(record)); }
  }
  ...
  private request<T>(storeName: string, mode: IDBTransactionMode, action: (store: IDBObjectStore) => IDBRequest): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      ...
      if (!transaction) { reject(new Error('Archive database closed')); return; }
      const request = action(transaction.objectStore(storeName)); let result: T;
...
function mergeById<T extends { id: string }>(before: readonly T[], after: readonly T[]): T[] {
  const merged = new Map(before.map((item) => [item.id, item])); for (const item of after) merged.set(item.id, item); return structuredClone([...merged.values()]);
}
...
export function computeCrossRunContext(records: readonly RunArchiveRecord[]): CrossRunContext {
  ...
  const tradeCounts = completed.map((record) => record.outcome.tradeRoutesEstablished); const exchangeCounts = completed.map((record) => record.outcome.knowledgeExchanges);
  const atomic = completed.filter((record) => record.outcome.atomicThresholdMonth !== undefined); const nuclearWeapons = completed.filter((record) => record.outcome.nuclearWeapons); const nuclearWars = completed.filter((record) => record.outcome.nuclearWar); const interplanetary = completed.filter((record) => record.outcome.interplanetary);
  ...
  const postBiologicalOrUnknown = completed.filter((record) => record.outcome.postBiological || record.outcome.unknown); const outcomeCounts: CrossRunContext['outcomeCounts'] = {};
  ...
  return {
    totalRuns: records.length, completedRuns: completed.length, industrializedRuns: industrialized.length,
    industrializedFraction: completed.length === 0 ? null : industrialized.length / completed.length,
    medianWritingYear: median(writingYears), medianWarsPerMillennium: median(warsPerMillennium), collapseFrequency: completed.length === 0 ? null : collapses.length / completed.length,
    tradeKnowledgeCorrelation: completed.length < 4 ? null : correlation(tradeCounts, exchangeCounts), atomicThresholdRuns: atomic.length,
    atomicThresholdFraction: completed.length === 0 ? null : atomic.length / completed.length, medianAtomicThresholdYear: median(atomic.map((record) => (record.outcome.atomicThresholdMonth ?? 0) / 12)),
    nuclearWeaponsRuns: nuclearWeapons.length, nuclearWeaponsFraction: completed.length === 0 ? null : nuclearWeapons.length / completed.length,
    nuclearWarRuns: nuclearWars.length, nuclearWarFraction: completed.length === 0 ? null : nuclearWars.length / completed.length,
    ...
    interplanetaryRuns: interplanetary.length, interplanetaryFraction: completed.length === 0 ? null : interplanetary.length / completed.length,
    extinctionOrCollapseRuns: extinctOrCollapsed.length, postBiologicalOrUnknownRuns: postBiologicalOrUnknown.length, outcomeCounts,
...
function median(values: readonly number[]): number | null {
  if (values.length === 0) return null; const sorted = [...values].sort((a, b) => a - b); const middle = Math.floor(sorted.length / 2);
...
function correlation(a: readonly number[], b: readonly number[]): number | null {
  ...
  const denominatorA = Math.sqrt(a.reduce((sum, value) => sum + (value - meanA) ** 2, 0)); const denominatorB = Math.sqrt(b.reduce((sum, value) => sum + (value - meanB) ** 2, 0));
